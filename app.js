...
(() => {
  ...
  let viewLimit = 120; // items per view (mobile friendly)
  ...
  function renderConteo(){
    ...
    const max = Math.max(40, viewLimit); // current page limit
    const slice = idx.slice(0, max);

    const total = idx.length;
    const showing = slice.length;

    if (els.showingHint){
      els.showingHint.textContent = total ? `Mostrando ${showing} de ${total}. Usá filtros/búsqueda para acotar.` : '';
    }
    if (els.loadMore){
      els.loadMore.disabled = showing >= total;
      els.loadMore.style.display = (total > showing) ? 'block' : 'none';
    }

    const renderRow = (r) => {
      const sku=norm(r.COD_ALFA), dep=norm(r.DEPOSITO), k=keyOf(sku,dep); const stock=numOr0(r.STOCK);
      const ex=edits.get(k);

      const paintDiff = (contadoStr, diffEl) => {
        if(contadoStr===undefined||contadoStr===''){diffEl.textContent='—'; diffEl.className=diffEl.className.replace(/\b(ok|bad|neu)\b/g,'').trim(); return;}
        const n=Number(contadoStr); if(!Number.isFinite(n)){diffEl.textContent='—'; return;}
        const d=n-stock; diffEl.textContent=String(d);
        // styles
        if (diffEl.classList){
          diffEl.classList.remove('ok','bad','neu');
          if (d===0) diffEl.classList.add('ok');
          else if (d<0) diffEl.classList.add('bad');
          else diffEl.classList.add('neu');
        }
      };
      ...
      if (!isMobile()){
        ...
        paintDiff(ex?.contado, diff);

        inp.addEventListener('input', async (ev)=>{ await onInput(ev.target.value); paintDiff(ev.target.value, diff); });

        row.appendChild(inp); row.appendChild(diff);
        card.appendChild(top); card.appendChild(row);
        els.cardlistConteo.appendChild(card);
      }
    };

    for(const i of slice){ renderRow(cuboRows[i]); }

    // keep the old table "more" row for desktop
    if(!isMobile() && total>showing){
      const tr=document.createElement('tr'); const td=document.createElement('td'); td.colSpan=6;
      td.innerHTML=`<span class="hint">Mostrando ${showing} de ${total}. Refiná con filtros o tocá “Cargar más”.</span>`;
      tr.appendChild(td); els.tbodyConteo.appendChild(tr);
    }
  }
