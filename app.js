...
function setOpts(sel,vals,labelAll){sel.innerHTML=''; const o=document.createElement('option');o.value='__ALL__';o.textContent=labelAll;sel.appendChild(o);
  ...
  async function uploadCubo(){await saveMeta(); const sid=els.sessionId.value.trim(); if(!sid) return status('Falta Session ID',true);
    ...
    try{status('Leyendo CSV…'); const parsed = await new Promise((res,rej)=>{
        let rows=[]; let seen=0;
        Papa.parse(f,{header:true,skipEmptyLines:true,worker:true,step:(results)=>{
          if(results && results.data){ rows.push(results.data); seen++; if(seen%400===0) status('Leyendo CSV… '+seen+' filas'); }
        },complete:()=>res(rows),error:rej});
      });
  ...
  els.createSession.addEventListener('click',createSession); if(els.uploadCubo) els.uploadCubo.addEventListener('click',uploadCubo); els.downloadCubo.addEventListener('click',downloadCubo);
  ...
  async function init(){
    ...
    if(els.apiUrl){ els.apiUrl.type='hidden'; }
    if(els.file){ els.file.style.display='none'; }
    if(els.uploadCubo){ els.uploadCubo.style.display='none'; }

    for(const s of [els.f_rubro,els.f_subrubro,els.f_filame,els.f_detalle,els.f_sku,els.f_deposito]) s.innerHTML='<option>—</option>';
    await loadMeta();
    if(!els.apiUrl.value.trim()) els.apiUrl.value = 'https://script.google.com/macros/s/AKfycbzWGGrh-QL-r8xLUKY0rQ8W9aSBUqiygqIrmuSkZxoec9zcp7LAxzpKY-KGxMTXPdZPeg/exec';
